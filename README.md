...
+ Define a function `addToCart`. This function should accept one argument, the item the user wants to purchase.
  - This function should automatically set a price for this item by generating a random number between 0 and 100. (***Hint***: `Math.random()` generates a random number between 0 (inclusive) and 1 (non-inclusive); `Math.floor()` rounds a number down to the nearest integer.)
